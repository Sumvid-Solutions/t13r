...
function abbreviateLineHtml(htmlContent) {
  var tmp = document.createElement('div');
  tmp.innerHTML = htmlContent; // safe: generated by our own renderer
      ...
      for (var j = 0; j < text.length; j++) {
        ...
        if (ch === '|' || ch === '।' || ch === '॥' || ch === '-') {
          ...
          if (inWord) {
            wordCount++;
            inWord = false;
            if (wordCount >= 2) collecting = false;
          }
...
function withLineNumbers(html, showToggle) {
  var rows = html.split('\n').map(function (line, idx) {
    if (showToggle) {
      var abbr = abbreviateLineHtml(line);
      return '<tr>' +
